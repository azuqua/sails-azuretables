...
module.exports = (function(){
  ...
  var adapter = {
    ...
    registerCollection: function(collection, cb) {
      ...
      console.log("Registering collection!", collection, schema);
      this.define(collection.identity, collection.definition, function(err, name){
        if(err){
          cb(err);
        }else{
          cb();
    ...
    find: function(collectionName, options, cb) {
      ...
      if(!check){
        ...
        console.log("Finding by: ", options);
                  ...
                  if(e){
                    ...
                    if(continuation && continuation.hasNextPage()){
                      utils.getPages(result, continuation, function(allResults){
                        console.log("Done getting pages...", allResults.length, allResults);
                        pool.release(ts);
                        utils.cleanAzureMetadata(allResults, cb);
                      });
                    }else{
                      console.log("Done looking up non-continued data.", result);
                      pool.release(ts);
    ...
    */
    ...
    update: function(collectionName, options, values, cb){
      ...
      var self = this;
      var keys = Object.keys(values);
      console.log("Updating: ", collectionName, options, values);
      if(!check){
        cb(new Error("Sorry, sails-azure doesn't natively support the '" + bad + "' operation. For the time being if you'd like to use this operation you'll have to implement it in your application logic."));
      }else{
